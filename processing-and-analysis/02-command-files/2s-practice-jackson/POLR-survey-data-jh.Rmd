...
```{r}
master <- readRDS(here(importable_data_dir, "two_stage_master_wide_deid.rds"))
```
...
In Jackson's original work, he (naturally) coded both the beginning- and end-of-term
survey responses as ordered factors for use in the POLR function. However, having 
and ordered factor as a predictor variable leads to very strange (according to Elli) 
model estimates with way too high t-values. 
...
The EoT survey response will be coded as an ordered factor in each case. Models for 
all three cases will be compared. 
...
model_1.1 <- polr(perts_q1msfixed2 ~ exp*sex_id +
                  exp + sex_id + perts_q1msfixed1 +
                  satm + satv + hs_gpa + aleksikc_score, data = master)

summary(model_1.1)

model_1.2 <- polr(perts_q1msfixed2 ~ exp*sex_id +
                  exp + sex_id +
                  satm + satv + hs_gpa + aleksikc_score, data = master)
