...
na_exp1 <- inst1_df2 %>% filter(is.na(experiment1)) %>% distinct(studID) 
na_proj1 <- inst1_df2 %>% filter(is.na(project1)) %>% distinct(studID) 
...
```{r}
inst1_df2 %>% select(studID, course.fullid, satmath) %>% 
  filter(is.na(satmath)) %>% 
  distinct(studID) %>% 
  tally()
```
There are 145 students who have `NA` for `satamth`. 

I will also have to track down the two-stage quiz data and combine it with the `inst1_df2`
data.

Save `inst1_df2` to the analysis-data folder:

```{r}
write_rds(inst1_df2, paste0(analysis_data_dir, "/inst1_df2.rds"))
```
...
in both 2016 and 2017, I will create a 2017_4-only df from `inst1_df2` and then 
...
inst1_df2 <- inst1_df2 %>% rename(exam_key = key)
two_stage <- two_stage %>% rename(quiz_key = key)
```


#### First, create year-specific versions of the `inst1_df2` data

Filter out the 2017_4 data.

```{r}
only_2017_4 <- 
  inst1_df2 %>% filter(str_detect(course.fullid, "2017_4"))
...
```{r}
master_two_stage <- read_csv(paste0(analysis_data_dir, "/master_two_stage.csv"))
